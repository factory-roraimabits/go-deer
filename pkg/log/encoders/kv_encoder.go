...
func (enc *kvEncoder) EncodeEntry(ent zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	...
	if ent.Stack != "" && final.StacktraceKey != "" {
		...
		// The next call will correctly indent the stack trace for printing.
		// The stack trace is being built in zap, and we don't want to fork zap just for this.
		final.addStackTrace(ent.Stack)
	}
...
// addStackTrace appends a stack trace string with the correct indenting.
func (enc *kvEncoder) addStackTrace(s string) {
	// start printing in a new line
	enc.buf.AppendString("\n\t")

	// correct indenting by adding a tab to all LF in stack trace.
	for i := 0; i < len(s)-1; i++ {
		switch s[i] {
		case '\n':
			enc.buf.AppendString("\n\t")
